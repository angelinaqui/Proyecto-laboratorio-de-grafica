...
int main() {
 ...
 //     
	...
	glm::mat4 projection=glm::mat4(1); //creamos la var de proyeccion que es una matriz 4x4, inicializamos en un 1 para que no se modifique nada
	...
	while (!glfwWindowShouldClose(window))
	{
	   ...
	   view = glm::translate(view, glm::vec3(0.0f,0.0f,-10.0f)); //traslada la vista, la vista es de 800x600
		//view = glm::translate(view, glm::vec3(400.0f, 300.0f, 0.0f)); //modificamos los valores de x y y para trasladar la vista
		...
		model = glm::mat4(1);
		model = glm::translate(view, glm::vec3(9.0f, 0.0f, 0.0f));
		model = glm::rotate(model, 0.5f, glm::vec3(1.0f, 1.0f, 0.0f));
		model = glm::scale(model, glm::vec3(3.5f, 3.5f, 2.0f));
		glUniformMatrix4fv(modelLoc, 1, GL_FALSE, glm::value_ptr(model));
		glDrawArrays(GL_TRIANGLES, 0, 36);
